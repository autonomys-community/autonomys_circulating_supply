...
export async function calculateCirculatingSupply() {
  try {
    ...
    const [consensusSupply, domainSupply, totalStaked, guardiansOfGrowthBalance] = await Promise.all([
    ...
    const circulatingSupply = totalOnChainSupply - totalStaked - lockedTokens - guardiansStakingIncentiveToVest;
