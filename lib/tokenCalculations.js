...
const ALLOCATIONS = {
  ...
  subspaceFoundation: {
    ...
    nearTermTreasury: { percent: 5.00, tokens: 50_000_000, vesting: 'locked' },
    longTermTreasury: { percent: 10.00, tokens: 100_000_000, vesting: 'locked' }
  },
  testnets: {
    testnetRewards: { percent: 5.97, tokens: 59_700_000, vesting: 'unlocked' },
...
export async function calculateCirculatingSupply() {
  try {
    ...
    const consensusSupply = await getConsensusTokenSupply();
    const domainSupply = await getDomainTokenSupply();
    ...
    const circulatingSupply = totalOnChainSupply - totalStaked - lockedTokens;
