...
import { getAmbassadorsWalletBalance, getAutoEvmNativeAi3BalanceAI3, getWrappedAi3BalanceOfAI3, getWrappedAi3TotalSupplyAI3 } from './ambassadorsService.js';
...
function calculateLockedTokens() {
  ...
  // Ambassadors are dynamic; do not include in static lockedAmount.
...
export async function calculateCirculatingSupply() {
  try {
    ...
    const [consensusSupply, domainSupply, totalStaked, guardiansOfGrowthBalance, nearTermTreasuryBalance, subspaceFoundationOperationsBalance, ambassadorsWalletBalance, wrappedAi3TotalSupply, hedgeySfAdminWrappedBalance, hedgeySfAdminNativeAi3Balance] = await Promise.all([
