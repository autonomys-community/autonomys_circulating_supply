...
import { getAmbassadorsWalletBalance } from './ambassadorsService.js';
import { getSubspaceFoundationAutoEvmTotalBalance, getWrappedAi3TotalSupply } from './subspaceFoundationAutoEvmService.js';
...
const ALLOCATIONS = {
  investors: { percent: 21.53, tokens: 215_263_087 },
  team: {
    foundersAndStaff: { percent: 7.14, tokens: 71_426_634 },
    advisors: { percent: 2.35, tokens: 23_478_000 }
  },
  autonomysLabs: {
    devcoTreasury: { percent: 7.00, tokens: 70_000_000 },
    marketLiquidity: { percent: 2.00, tokens: 20_000_000 }
  },
  vendors: { percent: 1.43, tokens: 14_345_400 },
  subspaceFoundation: {
    operations: { percent: 0.68, tokens: 6_782_580 },
    nearTermTreasury: { percent: 4.50, tokens: 45_000_000 },
    GuardiansOfGrowthStakingIncentive: { percent: 0.50, tokens: 5_000_000 },
    longTermTreasury: { percent: 10.00, tokens: 100_000_000 }
  },
  testnets: {
    testnetRewards: { percent: 5.97, tokens: 59_695_698 },
    stakeWars1: { percent: 0.60, tokens: 6_000_000 },
    stakeWars2: { percent: 0.30, tokens: 3_000_000 }
  },
  ambassadors: { percent: 1.00, tokens: 10_000_000 },
  farmerRewards: { percent: 35.00, tokens: 350_000_000 }
};
...
export async function calculateCirculatingSupply() {
  try {
    ...
    const [consensusSupply, domainSupply, totalStaked, guardiansOfGrowthBalance, nearTermTreasuryBalance, subspaceFoundationOperationsBalance, ambassadorsWalletBalance, sfAutoEvmBalance, wrappedAi3TotalSupply] = await Promise.all([
    ...
    // Fetch locked wallet balances separately to avoid overwhelming the RPC
    // endpoint with too many simultaneous WebSocket connections
    const lockedTokens = await getLockedWalletsConsensusTotalBalance();
    ...
    // Ambassadors:
    // - Tokens remaining in the ambassadors wallet on the consensus chain are
    //   locked (non-circulating).
    const ambassadorsAllocation = ALLOCATIONS.ambassadors.tokens;
    const ambassadorsLocked = Math.max(0, Math.min(ambassadorsAllocation, ambassadorsWalletBalance || 0));

    // Wrapped AI3:
    // - ALL wrapped AI3 (wAI3 ERC-20 totalSupply on Auto EVM) is non-circulating.
    // - sfAutoEvmBalance tracks NATIVE AI3 only, so there is no double-counting
