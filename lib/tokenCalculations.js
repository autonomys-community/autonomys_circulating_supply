...
export async function calculateCirculatingSupply() {
  try {
    ...
    const [consensusSupply, domainSupply, totalStaked, guardiansOfGrowthBalance, nearTermTreasuryBalance, subspaceFoundationOperationsBalance] = await Promise.all([
    ...
    const circulatingSupply = totalOnChainSupply - totalStaked - lockedTokens - guardiansStakingIncentiveToVest - nearTermTreasuryBalance - subspaceFoundationOperationsBalance;
