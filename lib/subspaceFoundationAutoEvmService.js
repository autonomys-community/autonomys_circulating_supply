...
export async function getSubspaceFoundationAutoEvmWalletBreakdown(wsUrl = AUTO_EVM_WS_URL) {
  ...
  try {
    ...
    const missing = addresses.filter(a => !balanceMap.has(a));
    console.warn(
      `Auto EVM batch incomplete for breakdown (${balanceMap.size}/${addresses.length}): ${batchError.message}. ` +
      `Retrying ${missing.length} wallet(s) individually.`
    );
    ...
    await Promise.all(missing.map(async (address) => {
      try {
        ...
        console.error(`Failed individual retry for breakdown ${address}: ${err.message}`);
      }
    }));
  }
