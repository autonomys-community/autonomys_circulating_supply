...
async function batchEthGetBalance(addresses, wsUrl = AUTO_EVM_WS_URL, timeoutMs = 15_000) {
  ...
  return new Promise((resolve) => {
    ...
    ws.onopen = () => {
      // Use a base ID with index offsets to guarantee uniqueness within the batch
    ...
    ws.onmessage = (event) => {
      try {
        ...
        // All responses received — close early
...
//
// IMPORTANT: Only wallets whose tokens are NOT already excluded elsewhere should
// be included in the active exclusion list. Wallets for statically-locked
// categories (Investors, Team/Advisors, Vendors, DevCo Treasury, Long-Term
// Treasury) are already subtracted via the hardcoded `lockedTokens` amount in
// tokenCalculations.js — including them here would double-count.

// Wallets already covered by static locks in calculateLockedTokens() — listed
// here for reference only (NOT queried).
export const SF_AUTO_EVM_WALLETS_STATIC_LOCKED = [
...
// Wallets that are dynamically tracked or represent new exclusions — these ARE
// queried and subtracted from circulating supply.
export const SF_AUTO_EVM_WALLETS = [
...
export async function getSubspaceFoundationAutoEvmTotalBalance(wsUrl = AUTO_EVM_WS_URL) {
  try {
    ...
    const balanceMap = await batchEthGetBalance(addresses, wsUrl);

    let total = 0;
    for (const { address, name } of SF_AUTO_EVM_WALLETS) {
      const wei = balanceMap.get(address) ?? 0n;
      total += roundWeiToWholeAI3(wei);
    }
    return total;
  } catch (error) {
    console.error('Error fetching Subspace Foundation Auto EVM balances:', error);
    return 0;
  }
}

/**
 * Get individual balances for each Subspace Foundation wallet on Auto EVM.
 * Useful for debugging and detailed breakdowns.
 *
 * @param {string} wsUrl - WebSocket JSON-RPC URL
 * @returns {Promise<Array<{address: string, name: string, balance: number}>>}
 */
export async function getSubspaceFoundationAutoEvmWalletBreakdown(wsUrl = AUTO_EVM_WS_URL) {
  try {
    const addresses = SF_AUTO_EVM_WALLETS.map(w => w.address);
    const balanceMap = await batchEthGetBalance(addresses, wsUrl);

    return SF_AUTO_EVM_WALLETS.map(({ address, name }) => ({
      address,
      name,
      balance: roundWeiToWholeAI3(balanceMap.get(address) ?? 0n),
    }));
  } catch (error) {
    console.error('Error fetching Subspace Foundation Auto EVM wallet breakdown:', error);
    return SF_AUTO_EVM_WALLETS.map(({ address, name }) => ({ address, name, balance: 0 }));
  }
}
