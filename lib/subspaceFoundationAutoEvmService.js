...
import { AUTO_EVM_WS_URL, roundWeiToWholeAI3, autoEvmEthGetBalance } from './autoEvmUtils.js';
...
export async function getSubspaceFoundationAutoEvmTotalBalance(wsUrl = AUTO_EVM_WS_URL) {
  try {
    const balances = await Promise.all(
      SF_AUTO_EVM_WALLETS.map(async ({ address, name }) => {
        try {
          const wei = await autoEvmEthGetBalance({ address, wsUrl });
          return roundWeiToWholeAI3(wei);
        } catch (error) {
          console.warn(`Error fetching balance for ${name} (${address}):`, error.message);
          return 0;
        }
      })
    );
  ...
  try {
    return await Promise.all(
      SF_AUTO_EVM_WALLETS.map(async ({ address, name }) => {
        try {
          const wei = await autoEvmEthGetBalance({ address, wsUrl });
          return { address, name, balance: roundWeiToWholeAI3(wei) };
        } catch (error) {
          console.warn(`Error fetching balance for ${name} (${address}):`, error.message);
          return { address, name, balance: 0 };
        }
      })
    );
  } catch (error) {
